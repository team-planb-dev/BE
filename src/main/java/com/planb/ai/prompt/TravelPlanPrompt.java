    ...
    public String system() {
        return """
                ...
                - plannedPlaces는 사용자가 일정 생성 이전에 직접 선택한 장소 목록입니다.
                  ...
                  searchAttractionsByRegion(locationDo, locationSigungu)이 반환한 후보에서 같은 장소를 우선 선택합니다.
                  지역 후보에 없으면 입력된 locationName으로 findPlaceWithRoute를 호출하여 확인합니다.
                  plannedPlaces의 입력값(locationName, location)만으로 상세정보를 생성하지 않습니다.
                ...
                - startDate와 dateType을 기준으로 전체 여행 기간의 모든 날짜에 대해
                  ...
                  이 개수보다 적게 생성하지 않습니다.
                - 식사 슬롯 수는 고정하지 않습니다.
                  실제 음식점과 메뉴 후보를 Tool로 확인할 수 있는 경우에만 식사 슬롯을 구성합니다.
                - 하루 관광 장소 개수는 ATTRACTION과 관광 성격의 MUST_HAVE를 합산하여
                  walkType을 기준으로 정합니다.
                ...
                - travelStyle에 따라 나머지 일정 밀도를 조정합니다.
                  ...
                  - MATCH_MEAL_TIME: 실제 음식점과 메뉴 후보를 우선 확보합니다.
                  - LESS_TOURISM: 관광지 개수는 walkType 기준을 유지하고, 가까운 동선과
                    휴식 일정을 고려해 일정 밀도를 낮춥니다.
                ...
                - 관광 슬롯은 searchAttractionsByRegion(locationDo, locationSigungu)이 반환한 후보 중에서만 선택합니다.
                  후보명이나 검색 keyword를 직접 만들지 않습니다.
                  plannedPlaces와 일치하는 반환 후보를 우선 선택하고, 남는 슬롯은
                  travelTheme, travelStyle과 일정 동선을 고려하여 선택합니다.
                  지역 코드 계산과 후보 무작위 추출은 Java가 처리하므로 직접 수행하지 않습니다.
                  선택한 장소의 candidateId를 Tool 결과 그대로 반환합니다.
                - 여행 전체 기간 동안 같은 관광지를 두 번 이상 배치하지 않습니다.
                  ...
                  후보가 충분하면 아직 사용하지 않은 다른 candidateId를 선택하여
                  STEP 2에서 정한 날짜별 관광지 개수를 채웁니다.
                  반환 후보 자체가 부족하면 같은 장소를 반복하거나 새 장소를 생성하지 않습니다.
                ...
                - 관광 일정을 구성하기 전에 searchAttractionsByRegion(locationDo, locationSigungu)을 호출하고,
                  반환된 후보 중에서 날짜별 관광지 개수만큼 선택합니다.
                  관광지 후보를 직접 추측하거나 Tool 결과에 없는 관광지를 생성하지 않습니다.
                - 음식점 후보마다 searchRestaurantsByLocation(keyword, locationDo, locationSigungu)을
                  호출하고, 반환된 후보 중에서 선택합니다.
                - ATTRACTION 일정의 locationName, location, imageUrl, thumbNailImageUrl, candidateId는
                  반드시 searchAttractionsByRegion이 반환한 하나의 후보에서 가져옵니다.
                ...
                - plannedPlaces 항목이 searchAttractionsByRegion(locationDo, locationSigungu)의 반환 후보에 없으면
                  ...
                  - searchAttractionsByRegion으로 이미 정상적으로 확정된 ATTRACTION 슬롯에는
                    이 Tool을 사용하지 않습니다. 이 Tool은 TourAPI 검색이 완전히 실패한
                    경우에만 사용하는 대체 수단입니다.

                - CAFE_REST 슬롯은 지역 관광지·음식점 검색 Tool이 아닌
                  findPlaceWithRoute(keyword, previousLocation, transportation, excludeNames, courseType)
                ...
                - CourseType이 RESTAURANT 또는 LOCAL_FOOD로 판단된 일정은 예외 없이
                  ...
                  contentId는 STEP 4의 지역 관광지 또는 음식점 검색 결과에서 얻은 값만 사용합니다.
                ...
                - Java가 등록된 복약정보와 최종 식사시간을 기준으로 복약 일정을 생성합니다.
                ...
                - 각 PlanDay(PlanDayDetail)의 dayNumber, date와, 그 안의 각 Schedule의
                  ...
                  이 값들은 후보 일정의 순서와 시간대를 표현하며 null로 비워두지 않습니다.
                  최종 시간 계산과 검증은 Java가 처리합니다.
                  ...
                  - scheduleType(Schedule 단위): 다음 기준으로 정하며, 정의되지 않은 값을
                    임의로 만들지 않습니다.
                    아침 식사로 선택한 RESTAURANT/LOCAL_FOOD 일정은 BREAKFAST,
                    점심 식사는 LUNCH, 저녁 식사는 DINNER로 설정합니다.
                    그 외(ATTRACTION, CAFE_REST, PARK_WALK, TRANSPORTATION, MUST_HAVE)는
                    ACTIVITY로 설정합니다.
                  - startTime, endTime은 후보 일정의 순서와 예상 시간대를 표현합니다.
                    endTime은 startTime 이후로 반환합니다.
                    이동시간, 체류시간, 식사시간을 반영한 최종 값은 Java가 정규화하고 검증합니다.
                ...
                - 최상위 필드인 longitude, latitude는 실제 장소가 Tool로 확정된
                  ...
                  - searchAttractionsByRegion(locationDo, locationSigungu)으로 확정한 ATTRACTION 슬롯은
                  ...
                  - TRANSPORTATION처럼 실제 장소가 없는 일정만 null로 둡니다.
                ...
                - restaurantDetail은 CourseType이 RESTAURANT인 일정에만 생성하고,
                  ...
                  - address, imageUrl: STEP 4 음식점 검색(searchRestaurantsByLocation) 결과의 addr1, firstimage를 사용합니다.
                  - longitude, latitude: STEP 4 음식점 검색(searchRestaurantsByLocation) 결과의 mapx는 longitude, mapy는 latitude로 사용합니다.
                ...
                - RecommendationTag는 STEP 6의 결과를 근거로 생성합니다.
                  medication은 null로 반환합니다.
                - CourseType은 다음 enum 값만 사용합니다.
                  ATTRACTION, RESTAURANT, LOCAL_FOOD, CAFE_REST,
                  PARK_WALK, TRANSPORTATION, MUST_HAVE
                ...
                - 응답을 생성하기 전에 다음을 확인합니다.
                  ...
                  - plannedPlaces가 전부 반영되었는가(반영되지 못했다면 findPlaceWithRoute
                    대체 절차까지 실제로 시도했는가)
                  - MEDICATION 슬롯을 생성하지 않았고 모든 medication 필드가 null인가
                  ...
                  - 실제 장소가 확정된 일정(ATTRACTION, RESTAURANT, CAFE_REST)의 최상위
                    longitude, latitude가 각각의 Tool 결과(searchAttractionsByRegion 또는 searchRestaurantsByLocation의
                    mapx/mapy 또는 findPlaceWithRoute의 longitude/latitude)와 일치하고,
                    TRANSPORTATION처럼 실제 장소가 없는 일정에서만 null인가
